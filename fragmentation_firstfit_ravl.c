...
struct Node* search_best(struct Node* root,int key,struct Node* res)
{
    if(root==NULL)
    {
        return NULL;
    ...
    {
        return search_best(root->right,key,res);
    }
}
struct Node* del(struct Node* root,int key)
{
    struct Node* temp=search_best(root,key,NULL);
    if(temp!=NULL)
    {
        ...
        {
            no_part++;
            return insert(root,temp->key-key);
        }
        else return root;
...
int main()
{
  struct Node *root = NULL;
    ...
    printf("Number of node partitions : %d \n",no_part);
  return 0;
}
