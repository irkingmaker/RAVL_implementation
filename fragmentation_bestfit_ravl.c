...
struct Node* search_best(struct Node* root,int key,int diff,struct Node* res)
{
    if(root==NULL)
    {
        return res;
    ...
    if(key < root->key)
    {
        int temp_diff=root->key-key;
        if(temp_diff<diff)
        {
            diff=temp_diff;
            res=root;
        }
        return search_best(root->left,key,diff,res);
    }
    else if(key > root->key)
    {
        return search_best(root->right,key,diff,res);
    }
    else
    {
       return root;
...
{
    struct Node* temp=search_best(root,key,INT_MAX,NULL);
    if(temp!=NULL)
    {
        float diff=temp->key-key;
        no_hits++;
        float diff_p=diff/temp->key;
        unused_memp+=diff_p;
        root=deleteNode(root,temp->key);
        if((int)diff!=0)
        {
                no_parts++;
                return insert(root,(int)diff);
        }
        else return root;
    }
    else
...
void preOrder(struct Node *root)
{
    if(root != NULL)
    {
        no_nodes+=root->count;
        printf("%d", root->key);
        if(root->count!=0) printf("(%d) ",root->count);
        preOrder(root->left);
        preOrder(root->right);
    }
...
int main()
{
  struct Node *root = NULL;
    ...
    printf("node miss :%d\n",miss);
    float avg_unused=unused_memp/no_hits;
    printf("average unused memory percentage : %Lf %\n",avg_unused*100);
    printf("Number of nodes in the tree :%d\n",no_nodes);
    printf("Number of nodes partitioned :%d\n",no_parts);
  return 0;
}
